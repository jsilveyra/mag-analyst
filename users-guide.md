...
- **PRIMA-BOBYQA, tuned npt** (default). A model-based trust-region method: each iteration builds a local quadratic model of the objective by interpolating it at a set of previously evaluated points, then minimizes that model exactly within the current trust region and the user's bounds, rather than searching heuristically. MagAnalyst's implementation (`src/lib/bobyqa_prima/`) follows Powell's BOBYQA algorithm [[22]](#22), reusing PRIMA's [[23]](#23) pure-MATLAB incremental model-update machinery together with an exact box-constrained trust-region solver. The number of interpolation points (`npt`), which trades model richness against evaluation cost, is chosen automatically from the number of fitted parameters $n$: a full quadratic model, $(n+1)(n+2)/2$ points, for $n \leq 6$, and the cheaper $2n+1$ points for $n > 6$. **This threshold is an empirical rule of thumb found for MagAnalyst's own anhysteretic fits, not a general theoretical result**: benchmarking against Nelder-Mead showed that, at $n=5$ (a two-component anhysteretic fit), the full-quadratic model reached the best accuracy in fewer evaluations than either $2n+1$ or Nelder-Mead, while at $n=11$ (a four-component fit) $2n+1$ reached roughly twice the accuracy of the full-quadratic model (or Nelder-Mead) at an equal evaluation budget, because the full-quadratic model's initialization cost had come to dominate that budget. The qualitative trade-off behind this rule — a richer interpolation model costs more to build and maintain, and that cost can dominate the evaluation budget as $n$ grows — is documented in the literature [[22]](#22)[[24]](#24), though neither source prescribes a universal $npt(n)$ rule; see `src/lib/bobyqa_prima/README.md` for the full benchmark data and references.
...
<br>
<a id="16">[16]</a>
J. D'Errico. interparc - Distance based interpolation along a general curve in space. Available: https://www.mathworks.com/matlabcentral/fileexchange/34874-interparc. Access date: 19/07/2026
<br>
<a id="17">[17]</a>
...
<br>
<a id="23">[23]</a>
Z. Zhang et al., PRIMA: Reference Implementation for Powell's methods with Modernization and Amelioration. Available: https://github.com/libprima/prima
<br>
<a id="24">[24]</a>
